...
class CircuitMPO:
    ...
    def init(self) -> List[jnp.ndarray]:
        ...
        return self.number_of_qubits * [
            jnp.eye(2, dtype=jnp.complex64).reshape((2, 1, 2, 1))
        ]

    def apply_gate(
        self, mpo: List[jnp.ndarray], gate: jnp.ndarray, sides: List[int], eps=1e-6
    ):
        """Update MPO inplace
        Args:
        ...
        right, left = right.reshape((2, -1, 2, right_bond_1)), left.reshape(
            (2, left_bond_0, 2, -1)
        )
        ...
        mpo[sides[0] + 1 : sides[1]] = list(
            map(
                partial(_mpo_block_eye_prod, eye_matrix=eye_matrix),
                mpo[sides[0] + 1 : sides[1]],
            )
        )

    def truncate(self, mpo: List[jnp.ndarray], eps=1e-6):
        ...
        for i in range(len(mpo) - 1):
            ...
            left, right = left.reshape((-1, left_shape[-1])), right.reshape(
                (right_shape[0], -1)
            )
            block = left @ right
            ...
            left, right = left.reshape(left_shape[:-1] + (-1,)), right.reshape(
                (-1,) + right_shape[1:]
            )
    ...
    def _push_sample(
        self, inp_sample: jnp.ndarray, mpo: List[jnp.ndarray], key: jnp.ndarray
    ) -> List[jnp.ndarray]:
        ...
        transfer_matrices = list(
            map(lambda x: jnp.einsum("ijk,ilm->jlkm", x, x.conj()), mps)
        )
        ...
        right_plugs = reduce(
            lambda x, y: [plug_update(y, x[0])] + x,
            transfer_matrices[::-1],
        ...
        def local_sample(x, y):
            ...
            new_ind = jnp.argmax(
                (random.gumbel(key, shape=(2,)) + log_p).astype(jnp.float32)
            )
        ...
        u, _, final_tensor, out_sample = reduce(
            local_sample,
            zip(mps, right_plugs),
            (jnp.array(0.0), key, jnp.ones((1,), dtype=jnp.complex64), []),
        )
    ...
    def push_sample(
        ...
        return self._push_sample(
            inp_sample, mpo, random.split(key, inp_sample.shape[0])
        )
